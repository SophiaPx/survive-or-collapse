#!/usr/bin/env bash
# DSL-B: proposer=intrinsic, solver=intrinsic, gate=off
# Key COLLAPSE condition. Mirrors code_o Run B.
set -euo pipefail
SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
source "${SCRIPT_DIR}/base.sh"

export RUN_GROUP="pred_dsl_o__solver-intrinsic_self_consistency__proposer-intrinsic-hard__program-off__qwen3-4b-base"
export EXP_NAME="selfplay_grpo_qwen3_4b_pred_dsl_o_solver_intrinsic_self_consistency_proposer_intrinsic_hard_program_off_${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)}"
