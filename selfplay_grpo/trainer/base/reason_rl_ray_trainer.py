...
class ReasonRLRayPPOTrainer(RayPPOTrainer):
    ...
    def _create_dataloader(self):
        ...
        # Optional dedicated codegen val dataloader (HumanEvalPlus / MbppPlus).
        # Triggered independently at trainer.codegen_val_freq.
        codegen_val_files = self.config.data.get('codegen_val_files', None)
        if codegen_val_files:
            self.codegen_val_dataset = RLHFDataset(parquet_files=codegen_val_files,
                                                   tokenizer=self.tokenizer,
                                                   prompt_key=self.config.data.prompt_key,
                                                   max_prompt_length=self.config.data.max_prompt_length,
                                                   filter_prompts=True,
                                                   ...
                                                   truncation='error',
                                                   extra_source_key="val")
            self.codegen_val_dataloader = StatefulDataLoader(dataset=self.codegen_val_dataset,
                                                             batch_size=len(self.codegen_val_dataset),
                                                             shuffle=False,
                                                             drop_last=True,
                                                             collate_fn=collate_fn)
            print(f'Size of codegen val dataloader: {len(self.codegen_val_dataloader)} (n_rows={len(self.codegen_val_dataset)})')
        else:
            self.codegen_val_dataset = None
            self.codegen_val_dataloader = None
    ...
    def fit(self):
        ...
        from selfplay_grpo.utils.logging_utils.stdout import PrettyPrinter as pp
