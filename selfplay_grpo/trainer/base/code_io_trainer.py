...
from selfplay_grpo.data_construction.constructor import get_gen_code_io_data, get_pred_code_io_data
from selfplay_grpo.trainer.base.reason_rl_ray_trainer import ReasonRLRayPPOTrainer
from selfplay_grpo.utils.dataset.rl_dataset import RLHFDataset
from selfplay_grpo.rewards.code_reward import parse_code_input_output, parse_inputs_message
from selfplay_grpo.utils.code_utils.python_executor import PythonExecutor
from selfplay_grpo.utils.code_utils.sandboxfusion_executor import SandboxfusionExecutor
from selfplay_grpo.utils.auxiliary import reflection_keywords
...
class CodeIORayPPOTrainer(ReasonRLRayPPOTrainer):
    _supported_tasks = {'code_i', 'code_o', 'code_e', 'code_f', 'dsl_o'}
    def __init__(self, past_epoch_window: int = 10, *args, **kwargs):
        super().__init__(*args, **kwargs)
        assert all(problem_type in self._supported_tasks for problem_type in self.config.selfplay.problem_types), \
            f"Invalid problem type: {self.config.selfplay.problem_types}"
        self._past_epoch_window = past_epoch_window
        if self.config.selfplay.executor == 'qwq':
            self._executor = PythonExecutor(
                timeout_length=self.config.selfplay.execute_max_timeout, 
                ast_check=self.config.selfplay.ast_check,
                max_workers=self.config.selfplay.get('executor_max_workers', 1)
            )
        elif self.config.selfplay.executor == 'sandboxfusion':
            self._executor = SandboxfusionExecutor(
                timeout_length=self.config.selfplay.execute_max_timeout,
                ast_check=self.config.selfplay.ast_check,
                max_workers=self.config.selfplay.get('executor_max_workers', 1),
                use_china_mirror=self.config.selfplay.get('use_china_mirror', True)
            )
        elif self.config.selfplay.executor == 'dsl':
            from selfplay_grpo.utils.dsl_utils.dsl_executor import DSLExecutor
            self._executor = DSLExecutor()
        else:
            raise ValueError(f'Invalid executor: {self.config.selfplay.executor}')
        self.dataset_manager = DatasetManager.remote()
        self._last_cleanup_step = 0
        self._cleanup_frequency = self.config.selfplay.get('executor_cleanup_frequency', 5)
    ...
    def _create_train_code_gen_dataloader(
        ...
        if dataset_key is None:
            if problem_type == 'code_i':
                dataset_key = 'input'
            elif problem_type in ('code_o', 'dsl_o'):
                dataset_key = 'output'
        ...
        if problem_type == 'code_f' and not seeding:
            if self.config.selfplay.gen_data_probabilities_strategy == 'step':
            ...
            weights = None
        elif self.config.selfplay.gen_data_probabilities_strategy == 'uniform':
            weights = [1.0] * len(io_data)
        elif self.config.selfplay.gen_data_probabilities_strategy == 'step':
            weights = [w + 1 for w in ray.get(self.dataset_manager.get_steps_dataset.remote(dataset_key))]
        else:
            raise ValueError(f"Unknown strategy: {self.config.selfplay.gen_data_probabilities_strategy}")
        ...
        gen_params = {
            ...
            'content_max_length': self.config.selfplay.data_selection_strategy.content_max_length,
            'io_n': 1 if problem_type == 'code_f' else self.config.selfplay.data_selection_strategy.io_n,
            'instruction_type': self.config.reward_fn.extraction_type,
            'output_path': parquet_path,
            'split': 'train',
            'tokenizer': self.tokenizer,
            'banned_keywords': self.config.selfplay.data_selection_strategy.banned_words,
            'banned_assertion_keywords': self.config.selfplay.data_selection_strategy.banned_keywords_for_errors_and_exceptions,
            'weights': weights,
            'enable_composite_function': self.config.selfplay.data_selection_strategy.composite_start_step > 0 and self.global_steps >= self.config.selfplay.data_selection_strategy.composite_start_step,
            'composite_function_n_min': self.config.selfplay.data_selection_strategy.composite_function_n_min,
            'composite_function_n_max': self.config.selfplay.data_selection_strategy.composite_function_n_max,
            'composite_chance': self.config.selfplay.data_selection_strategy.composite_chance,
            'remove_after_return': self.config.selfplay.reward.generation_reward_config.remove_after_return,
            'remove_input_from_snippet': self.config.selfplay.reward.generation_reward_config.remove_input_from_snippet,
            'include_references': self.config.selfplay.reward.generation_reward_config.include_references,
            'require_self_output': (
                self.config.selfplay.task == 'pred_code_o'
                and self.config.selfplay.proposer_reward_mode == 'intrinsic'
            ),
        }
        ...
        if problem_type == 'code_f':
            gen_params.update({
                'num_inputs': self.config.selfplay.data_selection_strategy.num_inputs,
            })
    ...
    def _create_train_code_pred_dataloader(self, problem_type: str, data_len: int) -> DataLoader:
        ...
        if self.config.selfplay.get('pred_exclude_seed', False):
            full_dataset = [e for e in full_dataset if not e.get('_is_seed', False)]

        strategy = self.config.selfplay.pred_data_mix_strategy
        ...
        with marked_timer(f'adv/{problem_type}', timing_raw):
            ...
            if problem_type.startswith('gen'):
                reward_fn_kwargs = {
                    ...
                    'banned_words': self.config.selfplay.data_selection_strategy.banned_words, # need this to check for banned words
            ...
            if valid_programs and self.config.selfplay.random_print_max_programs > 0:
                PrettyPrinter.section_header(f"New {problem_type} Programs")
                max_print = min(self.config.selfplay.random_print_max_programs, len(valid_programs))
                for program in random.sample(valid_programs, max_print):
                    ...
                    if 'code_f' not in problem_type:
                        ...
                        PrettyPrinter.status("THOUGHT", program['thought'], "info")
                    print("\n" + "-"*80 + "\n")
            if correct_predictions and self.config.selfplay.random_print_max_programs > 0:
                PrettyPrinter.section_header(f"New {problem_type} Programs")
                max_print = min(self.config.selfplay.random_print_max_programs, len(correct_predictions))
            ...
            if self.config.selfplay.data_selection_strategy.max_programs is not None and problem_type.endswith('code_i'):
                truncated_length, before_length = ray.get(self.dataset_manager.truncate_datasets.remote(self.config.selfplay.data_selection_strategy.max_programs, 'input'))
                PrettyPrinter.status("DATA", f"Truncated {truncated_length} programs from input dataset, max programs is {self.config.selfplay.data_selection_strategy.max_programs}, dataset size was {before_length} before truncation", "info")
            if self.config.selfplay.data_selection_strategy.max_programs is not None and problem_type.endswith('code_o'):
                truncated_length, before_length = ray.get(self.dataset_manager.truncate_datasets.remote(self.config.selfplay.data_selection_strategy.max_programs, 'output'))
                PrettyPrinter.status("DATA", f"Truncated {truncated_length} programs from output dataset, max programs is {self.config.selfplay.data_selection_strategy.max_programs}, dataset size was {before_length} before truncation", "info")
            if self.config.selfplay.data_selection_strategy.max_programs is not None and problem_type.endswith('code_e'):
                truncated_length, before_length = ray.get(self.dataset_manager.truncate_datasets.remote(self.config.selfplay.data_selection_strategy.max_programs, 'error'))
                PrettyPrinter.status("DATA", f"Truncated {truncated_length} programs from error dataset, max programs is {self.config.selfplay.data_selection_strategy.max_programs}, dataset size was {before_length} before truncation", "info")
            if self.config.selfplay.data_selection_strategy.max_programs is not None and problem_type.endswith('code_f'):
                truncated_length, before_length = ray.get(self.dataset_manager.truncate_datasets.remote(self.config.selfplay.data_selection_strategy.max_programs, 'problem'))
                PrettyPrinter.status("DATA", f"Truncated {truncated_length} programs from problem dataset, max programs is {self.config.selfplay.data_selection_strategy.max_programs}, dataset size was {before_length} before truncation", "info")
    ...
    def _init_seed_dataset(self, problem_types: List[str]) -> Tuple[List[Dict], List[Dict], List[Dict]]:
        ...
        target_size = self.config.selfplay.data_selection_strategy.data_len * self.config.selfplay.data_selection_strategy.seed_batch_factor
                ...
                for batch_dict in seed_dataloader:
                    ...
                    for output_text in output_texts:
                        success, result = parse_code_input_output(
                            ...
                            parse_output=False,
                            remove_after_return=self.config.selfplay.reward.generation_reward_config.remove_after_return,
                            remove_comments=self.config.selfplay.reward.generation_reward_config.remove_comments,
                            remove_print=self.config.selfplay.reward.generation_reward_config.remove_print,
                            reject_multiple_functions=self.config.selfplay.reward.generation_reward_config.reject_multiple_functions,
                            f_replace_location=self.config.selfplay.reward.generation_reward_config.f_replace_location,
                            reject_test_input_in_code=self.config.selfplay.reward.generation_reward_config.reject_test_input_in_code,
                            code_location=self.config.selfplay.reward.generation_reward_config.code_location,
                        )
                        if success:
                            code_validity, output = self._executor.check_all(
                                ...
                                banned_keywords_for_errors_and_exceptions=self.config.selfplay.data_selection_strategy.banned_keywords_for_errors_and_exceptions,
                            )
                            if code_validity:
                    ...
                    if self.config.selfplay.data_selection_strategy.get('generate_seed_dataset_only', False):
                        with open(self.config.selfplay.data_selection_strategy.output_seed_path.replace('.jsonl', f'_temp.jsonl'), 'a') as f:
            ...
            while len(code_f_dataset) < target_size:
                ...
                for batch in code_f_seed_dataloader:
                    ...
                    for idx, output_text in enumerate(output_texts):
                        success, result = parse_inputs_message(output_text, num_inputs=self.config.selfplay.data_selection_strategy.num_inputs)
                        if success:
                            outputs = []
                ...
                if self.config.selfplay.data_selection_strategy.get('generate_seed_dataset_only', False):
                    with open(self.config.selfplay.data_selection_strategy.output_code_f_seed_path.replace('.jsonl', f'_temp.jsonl'), 'a') as f:
        ...
        if ('code_i' in problem_types or 'code_o' in problem_types) and self.config.selfplay.output_seed_path is not None:
            PrettyPrinter.status("DATASET", "Writing seed dataset to JSONL file...", "info")
            output_path = Path(self.config.selfplay.output_seed_path)
        ...
        if 'code_e' in problem_types and self.config.selfplay.output_error_seed_path is not None:
            PrettyPrinter.status("DATASET", "Writing error seed dataset to JSONL file...", "info")
            output_path = Path(self.config.selfplay.output_error_seed_path)
            output_path.parent.mkdir(parents=True, exist_ok=True)
            with open(output_path, 'w') as f:
        ...
        if 'code_f' in problem_types and self.config.selfplay.output_code_f_seed_path is not None:
            PrettyPrinter.status("DATASET", "Writing code f seed dataset to JSONL file...", "info")
            output_path = Path(self.config.selfplay.output_code_f_seed_path)
            output_path.parent.mkdir(parents=True, exist_ok=True)
            with open(output_path, 'w') as f:
        ...
        if 'code_i' in problem_types or 'code_o' in problem_types:
            ...
            PrettyPrinter.table(
                ...
                [
                    ...
                    ["Storage Path", self.config.selfplay.output_seed_path],
            ...
            # sample 3 entries
            for i, item in enumerate(random.sample(seed_dataset, self.config.selfplay.random_print_max_programs)):  
        ...
        if 'code_e' in problem_types:
            ...
            PrettyPrinter.table(
                ...
                [
                    ...
                    ["Target Size", target_size],
                    ["Storage Path", self.config.selfplay.output_error_seed_path],
            ...
            # sample 3 entries
            for i, item in enumerate(random.sample(error_dataset, self.config.selfplay.random_print_max_programs)):  
                PrettyPrinter.code_block(item['snippet'], "python")
                PrettyPrinter.status("INPUT", item['input'], "info")
                PrettyPrinter.status("OUTPUT", item['output'], "info")
                if i < 2:  # Don't print separator after last item
        ...
        if 'code_f' in problem_types:
            ...
            PrettyPrinter.table(
                ...
                [
                    ...
                    ["Target Size", target_size],
                    ["Storage Path", self.config.selfplay.output_code_f_seed_path],
            ...
            # sample 3 entries
            for i, item in enumerate(random.sample(code_f_dataset, self.config.selfplay.random_print_max_programs)):  
                PrettyPrinter.code_block(item['snippet'], "python")
    ...
    def fit(self):
        ...
        if self.loaded_datasets:
            ...
            # create init dataset
            need_seed_dataset = any(problem_type != 'code_e' for problem_type in self.config.selfplay.problem_types) or 'code_f' in self.config.selfplay.problem_types
            need_error_dataset = 'code_e' in self.config.selfplay.problem_types
            need_code_f_dataset = 'code_f' in self.config.selfplay.problem_types
            ...
            if need_seed_dataset:
                if self.config.selfplay.seed_dataset is not None:
                    ...
                    seed_dataset = seed_dataset[:self.config.selfplay.data_selection_strategy.data_len * 
                                                self.config.selfplay.data_selection_strategy.seed_batch_factor]
                    PrettyPrinter.status("DATA", f"Loaded {len(seed_dataset)} seed entries", "success")
                    if 'code_f' in self.config.selfplay.problem_types: # we need seed to generate code_f
            ...
            if need_error_dataset:
                if self.config.selfplay.error_seed_dataset is not None:
                    PrettyPrinter.status("DATA", "Loading error seed dataset from file...", "info")
                    with open(self.config.selfplay.error_seed_dataset, 'r') as file:
                        error_dataset = [json.loads(line) for line in file]
                    error_dataset = error_dataset[:self.config.selfplay.data_selection_strategy.data_len * 
                                                self.config.selfplay.data_selection_strategy.seed_batch_factor]
            ...
            if need_code_f_dataset:
                if self.config.selfplay.code_f_seed_dataset is not None:
                    ...
                    code_f_dataset = code_f_dataset[:self.config.selfplay.data_selection_strategy.data_len * 
                                                self.config.selfplay.data_selection_strategy.seed_batch_factor]
            ...
            if need_to_generate_seed or need_to_generate_error or need_to_generate_code_f:
                ...
                for problem_type in self.config.selfplay.problem_types:
                ...
                if self.config.selfplay.get('generate_seed_dataset_only', False):
                    return

            # Now initialize datasets in dataset manager
            if need_seed_dataset:
                assert len(seed_dataset) >= self.config.selfplay.data_selection_strategy.data_len

                if 'code_i' in self.config.selfplay.problem_types:
                    # Process locally first
                ...
                if 'code_o' in self.config.selfplay.problem_types:
                    processed_seed_dataset = process_elements(seed_dataset)
            ...
            if need_error_dataset:
                assert len(error_dataset) >= self.config.selfplay.data_selection_strategy.data_len
            ...
            if need_code_f_dataset:
                assert len(code_f_dataset) >= self.config.selfplay.data_selection_strategy.data_len
        ...
        self.global_steps += 1
        if self.config.selfplay.pretrain_pred_steps > 0 and self.global_steps <= self.config.selfplay.pretrain_pred_steps:
        ...
        while self.global_steps < self.total_training_steps:
            ...
            if self.config.selfplay.data_selection_strategy.composite_scheduler.enabled:
                self.scheduler_step()
            ...
            if 'code_e' in self.config.selfplay.problem_types:
            ...
            if 'code_f' in self.config.selfplay.problem_types:
            ...
            for _ in range(self.config.selfplay.data_selection_strategy.update_iteration):
                metrics = {}
                timing_raw = {}
                batches = {}
                with marked_timer('step', timing_raw):
                    ...
                    if 'code_i' in self.config.selfplay.problem_types:
                        if not self.pretrain_pred:
                            ...
                            if self.config.selfplay.train_propose:
                ...
                if self.global_steps >= self.config.selfplay.pretrain_pred_steps:
                    self.pretrain_pred = False
    ...
    def _load_checkpoint(self):
        ...
        if self.global_steps == 0:
            ckpt_path = self.config.trainer.get("load_checkpoint_path", None)
            if ckpt_path:
                match = re.search(r"global_step_(\d+)", str(ckpt_path))
                if match:
                    self.global_steps = int(match.group(1))
                    PrettyPrinter.status(
                        "RESUME",
                        f"Inferred global_steps={self.global_steps} from {ckpt_path}",
                        "info",
                    )
        if self.global_steps == 0:
    ...
    def scheduler_step(self):
        if self.config.selfplay.data_selection_strategy.composite_scheduler.enabled:
            ...
            if self.global_steps >= self.config.selfplay.data_selection_strategy.composite_scheduler.update_num_programs_start:
                steps_since_start = self.global_steps - self.config.selfplay.data_selection_strategy.composite_scheduler.update_num_programs_start
                num_updates = steps_since_start // self.config.selfplay.data_selection_strategy.composite_scheduler.update_num_programs_interval

                # Calculate new value directly from initial value + increments
                initial_max = self.config.selfplay.data_selection_strategy.max_programs_initial
                new_max = min(initial_max + num_updates, self.config.selfplay.data_selection_strategy.composite_scheduler.num_programs_max)

                # Only log if value changed
                if new_max != self.config.selfplay.data_selection_strategy.composite_function_n_max:
                    current_max = self.config.selfplay.data_selection_strategy.composite_function_n_max
                    self.config.selfplay.data_selection_strategy.composite_function_n_max = new_max
            ...
            if self.global_steps >= self.config.selfplay.data_selection_strategy.composite_scheduler.update_probability_start:
                steps_since_start = self.global_steps - self.config.selfplay.data_selection_strategy.composite_scheduler.update_probability_start
                num_updates = steps_since_start // self.config.selfplay.data_selection_strategy.composite_scheduler.update_probability_interval

                # Calculate new value directly from initial value + increments
                initial_prob = self.config.selfplay.data_selection_strategy.composite_chance_initial
                new_prob = min(initial_prob + (num_updates * self.config.selfplay.data_selection_strategy.composite_scheduler.update_probability_increment),
                              self.config.selfplay.data_selection_strategy.composite_scheduler.update_probability_max)

                # Only log if value changed
                if new_prob != self.config.selfplay.data_selection_strategy.composite_chance:
                    current_prob = self.config.selfplay.data_selection_strategy.composite_chance
                    self.config.selfplay.data_selection_strategy.composite_chance = new_prob
