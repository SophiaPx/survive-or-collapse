...
- Format multiple arguments with commas between them
- Remember to add quotes around string arguments
{self_output_requirement_prompt}
...
self_output_requirement_prompt = "- Also provide the self-output the code returns on that exact input.\n"

self_output_format_prompt = """- Format your self-output with:
```output
...
```
"""
...
The DSL uses prefix-notation expression trees with the following operators:

Binary operators (two sub-expressions):
  ADD(a,b)=a+b   SUB(a,b)=a-b   MUL(a,b)=a*b
  DIV(a,b)=a//b  MOD(a,b)=a%b   MAX(a,b)   MIN(a,b)

Unary operators (one sub-expression):
  NEG(a)=-a   ABS(a)=|a|

Conditional (if-then-else):
  ITE(cond, then_expr, else_expr)

Condition operators (used only inside ITE):
  GT(a,b)=a>b   LT(a,b)=a<b   EQ(a,b)=a==b   GEQ(a,b)=a>=b   LEQ(a,b)=a<=b

Variables: x, y  (integers)
Literals:  any signed integer (e.g. -3, 0, 7)
Output:    always a single integer
"""

dsl_output_prompt = """\
## Task: Create a New DSL Expression with One Matching Input

Using the reference DSL expressions provided below as examples, design a new and unique DSL expression tree that demands step-by-step reasoning to evaluate. Your submission should include a DSL expression and a test input pair, where the input will be plugged into the expression to produce the output. The input will be given to a test subject to deduce the output, which is meant to be an I.Q. test.

""" + DSL_LANGUAGE_SPEC + """
### Expression Requirements:
- Use only the operators listed above
- Use variables x and y (integers)
- The expression must always evaluate to a single integer for your chosen inputs
- Make the expression challenging to evaluate mentally by nesting operators to a depth of 3 or more
- Avoid trivially simple expressions (depth < 2)
- Choose inputs x, y such that the expression evaluates without division or modulo by zero
- The expression should require multiple sequential computation steps

### Formatting:
Format your DSL expression with:
```dsl
...
x=<integer>, y=<integer>
```
Format your self-output (the correct integer result) with:
```output
<integer>
```
...
### Example Format:
```dsl
ITE(GT(x,0),MUL(x,y),NEG(ADD(x,y)))
```
```input
x=3, y=-2
```
...
First, carefully plan what computational structure your expression will explore, then write the expression, input, and output.

### Reference DSL Expressions:
"""

dsl_output_predictor_prompt = """\
# Task: Evaluate a DSL Expression Given the Expression and Input

Given the following DSL expression and input values, think step by step to evaluate the expression and produce the integer output. Place your final integer answer in ```output``` tags.
...
def get_code_problem_generator_prompt(
    ...
    require_self_output: bool = False,
) -> str:
    # assert not (remove_after_return and not remove_input_from_snippet)
    composite_functions = list(composite_functions or [])
    snippet_string = ""
    ...
    if problem_type == "code_i":
        ...
        return code_output_prompt.format(
            remove_after_return_prompt=(remove_singleton_variables_prompt if remove_after_return else '\n'),
            remove_input_from_snippet_prompt=(remove_input_from_snippet_prompt if remove_input_from_snippet else ''),
            self_output_requirement_prompt=(self_output_requirement_prompt if require_self_output else ''),
            self_output_format_prompt=(self_output_format_prompt if require_self_output else ''),
            self_output_example_prompt=(self_output_example_prompt if require_self_output else ''),
        ).replace(
            '<|BANNED_KEYWORDS|>', ', '.join(banned_keywords)
        ) + snippet_string + (
            composite_requirements_prompt.format(
        ...
        return dsl_output_prompt + snippet_string
    else:
...
def get_code_problem_predictor_prompt(problem_type: str, snippet: str, input_args: str = None, output: str = None, message: str = None, input_output_pairs: List[Tuple[str, str]] = None) -> str:
    if problem_type.endswith("code_i"):
        ...
        return dsl_output_predictor_prompt.format(snippet=snippet, input_args=input_args)
    else:
        raise ValueError(f"Invalid problem type: {problem_type}")
