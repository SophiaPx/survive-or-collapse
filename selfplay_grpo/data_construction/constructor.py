...
from selfplay_grpo.data_construction.prompts import get_code_problem_generator_prompt, get_code_problem_predictor_prompt
from selfplay_grpo.data_construction.process_data import boxed_instruction, instruction_following
from selfplay_grpo.utils.code_utils.parsers import replace_main_function_name
...
def get_gen_code_io_data(
    ...
    while len(return_io_data) < target_data_len:
        ...
        # composite functions is not used for code_f or dsl_o problem types
        if problem_type not in ('code_f', 'dsl_o') and composite_function_n_max > 0 and enable_composite_function and random.random() <= composite_chance and len(chosen_references) > composite_function_n_max:
        ...
        io_prompt = instruction_template.format(
            get_code_problem_generator_prompt(
                ...
                require_self_output=require_self_output,
            )
        )
